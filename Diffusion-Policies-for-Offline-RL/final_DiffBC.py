...
def train_agent_and_collect_rewards(env, state_dim, action_dim, max_action, device, output_dir, args):
    ...
    data_sampler = Data_Sampler(dataset, device, args.reward_tune) # reward_tune is still used for data sampling, even if not directly for BC loss
    utils.print_banner('Loaded buffer')

    # CHANGED: Import Diffusion_BC instead of Diffusion_QL
    from agents.bc_diffusion import Diffusion_BC as Agent
    agent = Agent(state_dim=state_dim,
                  action_dim=action_dim,
                  max_action=max_action,
                  device=device,
                  discount=args.discount, # Discount is not used in BC, but keeping for compatibility if other parts rely on it
                  tau=args.tau, # Tau is not used in BC, but keeping for compatibility if other parts rely on it
                  beta_schedule=args.beta_schedule,
                  n_timesteps=args.T,
                  lr=args.lr) # CHANGED: Removed ql-specific parameters like max_q_backup, eta, lr_decay, lr_maxt, grad_norm
    ...
    while training_iters < max_timesteps:
        ...
        # CHANGED: Removed QL Loss, Actor Loss, Critic Loss as they are not relevant for BC
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr_decay", action='store_true') # Still present in args, but not directly used by BC_Diffusion
    parser.add_argument('--early_stop', action='store_true')
    parser.add_argument('--save_best_model', action='store_true')

    parser.add_argument("--discount", default=0.99, type=float) # Still present in args, but not directly used by BC_Diffusion
    parser.add_argument("--tau", default=0.005, type=float) # Still present in args, but not directly used by BC_Diffusion

    parser.add_argument("--T", default=5, type=int)
    parser.add_argument("--beta_schedule", default='vp', type=str)

    parser.add_argument("--algo", default="bc", type=str) # CHANGED: Default algorithm to 'bc'
    ...
    if args.env_name in default_hyperparameters:
        ...
        args.lr = hparams['lr']
        # CHANGED: Removed ql-specific hparams assignments (eta, max_q_backup, gn, top_k)
    ...
    # CHANGED: Check for 'bc' algorithm
    if args.algo == 'bc':
        ...
        raise ValueError(f"Algorithm '{args.algo}' is not supported in this custom script. Only 'bc' is implemented.") # CHANGED: Error message

    rewards_file_path = os.path.join(args.output_dir, "DiffBC_results.txt") # CHANGED: Output file name
    ...
    trained_agent.save_model(args.output_dir, "final_bc_model")
    print(f"Final trained model saved to {args.output_dir}/actor_final_bc_model.pth")