...
parser.add_argument("--exp_name", default='edp_four_corners_test', type=str) # 实验名称更新
parser.add_argument("--eta", default=1.0, type=float)
parser.add_argument('--device_id', default=0, type=int)
parser.add_argument("--results_dir", default='toy_results_edp_corners', type=str) # 结果目录更新
...
class Data_Sampler:
    def __init__(self, state, action, reward, device_val):
        self.state = state.to(device_val).float()
        self.action = action.to(device_val).float()
        self.reward = reward.to(device_val).float()
        self.num_samples = self.state.shape[0]
        self.device = device_val
        self.next_state = state.clone().to(device_val).float()
        self.not_done = torch.ones(self.num_samples, 1, device=device_val).float()
...
def generate_data(num, device='cpu'): # 新的 generate_data 函数
    if num <= 0:
        return Data_Sampler(state=torch.empty((0,2),dtype=torch.float32,device=device),
                            action=torch.empty((0,2),dtype=torch.float32,device=device),
                            reward=torch.empty((0,1),dtype=torch.float32,device=device),
                            device_val=device)

    each_num = int(num / 4)
    remaining_samples = num - (each_num * 4)

    pos = 0.8
    std = 0.05
    left_up_conor = Normal(torch.tensor([-pos, pos], device=device, dtype=torch.float32), torch.tensor([std, std], device=device, dtype=torch.float32))
    left_bottom_conor = Normal(torch.tensor([-pos, -pos], device=device, dtype=torch.float32), torch.tensor([std, std], device=device, dtype=torch.float32))
    right_up_conor = Normal(torch.tensor([pos, pos], device=device, dtype=torch.float32), torch.tensor([std, std], device=device, dtype=torch.float32))
    right_bottom_conor = Normal(torch.tensor([pos, -pos], device=device, dtype=torch.float32), torch.tensor([std, std], device=device, dtype=torch.float32))

    n_lu, n_lb, n_ru, n_rb = each_num, each_num, each_num, each_num
    if remaining_samples > 0: n_lu += remaining_samples

    left_up_samples = left_up_conor.sample((n_lu,)).clip(-1.0, 1.0)
    left_bottom_samples = left_bottom_conor.sample((n_lb,)).clip(-1.0, 1.0)
    right_up_samples = right_up_conor.sample((n_ru,)).clip(-1.0, 1.0)
    right_bottom_samples = right_bottom_conor.sample((n_rb,)).clip(-1.0, 1.0)

    data = torch.cat([left_up_samples, left_bottom_samples, right_up_samples, right_bottom_samples], dim=0)
    action = data.to(device).float()
    state = torch.zeros_like(action, device=device).float()

    r_left_up = 3.0 + 0.5 * torch.randn((n_lu, 1), device=device)
    r_left_bottom = 0.5 * torch.randn((n_lb, 1), device=device)
    r_right_up = 1.5 + 0.5 * torch.randn((n_ru, 1), device=device)
    r_right_bottom = 5.0 + 0.5 * torch.randn((n_rb, 1), device=device)
    reward = torch.cat([r_left_up, r_left_bottom, r_right_up, r_right_bottom], dim=0).to(device).float()

    return Data_Sampler(state=state, action=action, reward=reward, device_val=device) # 确保 device_val 参数名匹配
...
print(f"Generating training data (Four Corners Dataset - {num_data_points} samples)...")
data_sampler_instance = generate_data(num=num_data_points, device=device)
...
print(f"Starting EDP training on Four Corners Dataset for {num_epochs} epochs (K={diffusion_timesteps_K})...")
actor_loss_history, critic_loss_history = [], []
if data_sampler_instance.num_samples > 0:
    for epoch in range(1, num_epochs + 1):
        avg_actor_loss, avg_critic_loss = edp_agent_instance.train(data_sampler_instance, iterations_per_epoch, batch_size)
    ...
    print("EDP training on Four Corners Dataset finished.")
else:
...
axs[0].set_title('Ground Truth: Actions Colored by Reward (Four Corners)', fontsize=14)
axs[0].set_xlabel('Action Dimension 1', fontsize=12)
axs[0].set_ylabel('Action Dimension 2', fontsize=12)
...
axs[1].set_xlabel('Action Dimension 1', fontsize=12); axs[1].set_ylabel('Action Dimension 2', fontsize=12)
...
fig.suptitle(f'EDP Agent on Four Corners Dataset (Seed: {seed}, K={diffusion_timesteps_K}, λ={lambda_policy_loss_weight})', fontsize=16, y=0.99)
plt.tight_layout(rect=[0, 0.02, 1, 0.95])
file_name = f'edp_four_corners_K{diffusion_timesteps_K}_lambda{lambda_policy_loss_weight}_seed{seed}.pdf'
plt.savefig(os.path.join(img_dir, file_name))
print(f"Plot saved to {os.path.join(img_dir, file_name)}")
plt.show()

print("EDP four corners data test script finished.")